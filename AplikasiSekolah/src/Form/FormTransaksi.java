    ...
    private void btn_okActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btn_okActionPerformed
        ...
        try{
             String SUrl, SUser, SPass;
                        ...
                        Class.forName("com.mysql.cj.jdbc.Driver");
                        Connection con = DriverManager.getConnection(SUrl, SUser, SPass);
                        PreparedStatement ps = con.prepareStatement("select * from transaksi where nis = ?");
                        ps.setString(1, nis);
                ...
                if (!rs.next()) {
                    ...
                    String tambah ="INSERT INTO transaksi (tanggal_pembayaran,nis,nama,kelas,tahun_ajaran,total_tunggakan,total_pembayaran) VALUES (?,?,?,?,?,?,?)";
                    PreparedStatement pstt = conn.prepareStatement(tambah);
                    pstt.setString(1, date);
                    pstt.setString(2, nis);
                    pstt.setString(3, nama);
                    pstt.setString(4, kelas);
                    pstt.setString(5, tahun_ajaran);
                    pstt.setString(6, tunggakan);
                    pstt.setString(7, "0");

                    int rowInserted = pstt.executeUpdate();
...
//                        resetForm();
                    ...
                    String totalBayar = rs.getString("total_pembayaran");
                    String nisKedua = rs.getString("nis");
                    String namaKedua = rs.getString("nama");
                    String update ="UPDATE transaksi\n" + "SET tanggal_pembayaran = ?, nis = ?, nama = ?, kelas = ?, " +
                    ...
                    pst.setString(7, totalBayar);
                    pst.setString(8, nisKedua);
                    pst.setString(9, namaKedua);

                    int rowInserted = pst.executeUpdate();

                    if (rowInserted >0) {
                        JOptionPane.showMessageDialog(this, "Tunggakan Berhasil Diubah");
//                        resetForm();
                        getData();
                    }
                    pst.close();
